...
float sdf(vec3 p, mat3 rotate) {
    p *= rotate;
    mat3 spin = rotateX(uniformParams.iTime);
    vec3 dEye1pos = vec3(0, 0, -2);
    vec3 dEye2pos = vec3(0, 0, 2);
    
    float outSdf = dEye((p - dEye1pos) * spin + dEye1pos, dEye1pos, 0.5, 1.0 + pow(uniformParams.iTime, 2.0) * 0.01, 5.0);
    outSdf = min(outSdf, dEye((p - dEye2pos) * spin + dEye2pos, dEye2pos, 0.5, 1.0 + pow(uniformParams.iTime, 2.0) * 0.01, 5.0));
...
void main()
{
  vec2 iResolution = vec2(uniformParams.iResolution_x, uniformParams.iResolution_y);
  vec2 iMouse = vec2(uniformParams.iMouse_x, uniformParams.iMouse_y);
  ...
  vec3 cP = wP * rotate;
  vec3 color = vec3(0.5, asin(cos(uv * acos(-1.0) * 0.4))) * 2.0 * rotateX(uniformParams.iTime);
  if (hit) {
